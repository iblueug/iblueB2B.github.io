...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Check if there's a pre-selected supplier from URL
...
async function checkAuth() {
    try {
        ...
        if (!user) {
            window.location.href = 'login.html?redirect=create-po.html';
            return;
        }
        currentUser = user;
...
async function loadSuppliers() {
    ...
    try {
        ...
        if (!suppliers || suppliers.length === 0) {
            container.innerHTML = '<p class="no-results">No suppliers found. You can add manually below.</p>';
            return;
        }
        ...
        container.innerHTML = suppliers.map(supplier => {
            ...
            return `
                <div class="supplier-item" data-supplier-id="${supplier.id}" onclick="selectSupplier(${supplier.id})">
                    ...
                    <div class="supplier-info">
                        ...
                        <div class="supplier-meta">
                            <span><i class="fas fa-map-marker-alt"></i> ${escapeHtml(profile.location || 'Uganda')}</span>
...
window.selectSupplier = function(supplierId) {
    ...
    document.querySelectorAll('.supplier-item').forEach(item => {
        item.classList.toggle('selected', item.dataset.supplierId == supplierId);
    });
...
window.selectSupplierById = async function(supplierId) {
    ...
    if (supplier) {
        ...
        // Update to step 1 if we're on it
        if (currentStep === 1) {
            document.querySelectorAll('.supplier-item').forEach(item => {
                item.classList.toggle('selected', item.dataset.supplierId == supplierId);
            });
...
window.saveManualSupplier = function() {
...
window.setCartItemQuantity = function(index, value) {
    if (index < 0 || index >= cart.length) return;
...
function validateStep1() {
    ...
    continueBtn.disabled = !selectedSupplier;
}
...
function validateStep3() {
    ...
    poData.paymentTerms = document.getElementById('paymentTerms')?.value || 'advance_full';
    poData.deliveryAddress = document.getElementById('deliveryAddress')?.value || '';
    poData.deliveryDistrict = document.getElementById('deliveryDistrict')?.value || '';
    poData.deliveryPhone = document.getElementById('deliveryPhone')?.value || '';
    
    // Validate required fields
    const isValid = poData.paymentMethod && poData.deliveryAddress;
...
async function submitPurchaseOrder() {
    ...
    try {
        ...
        if (isManualSupplier) {
            // Create temporary supplier record
        ...
        const { data: order, error: orderError } = await sb
            ...
            .insert({
                ...
                delivery_district: poData.deliveryDistrict,
                delivery_contact_phone: poData.deliveryPhone,
                internal_reference: poData.internalReference,
                supplier_notes: poData.supplierNotes,
        ...
        // Create notification for supplier (if not manual)
        if (!isManualSupplier) {
            await sb
                .from('notifications')
                .insert({
        ...
        // Show success modal
        document.getElementById('poNumberDisplay').textContent = poNumber;
        document.getElementById('viewPOBtn').href = `purchase-order.html?id=${order.id}`;
        document.getElementById('successMessage').textContent = 'Your purchase order has been created and sent to the supplier.';
...
function setupEventListeners() {
    ...
    document.getElementById('scanBarcodeBtn')?.addEventListener('click', () => {
        ...
        // Initialize scanner (simplified - you'd need actual barcode library)
        setTimeout(() => {
            showToast('Scanner ready - position barcode');
        }, 500);
    });
...
window.saveManualSupplier = saveManualSupplier;
